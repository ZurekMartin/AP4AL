...
int main(void)
{
  ...
  List_t seznam1, seznam2, vysledek;
  List_Init(&seznam1);
  List_Init(&seznam2);
  List_Init(&vysledek);

  Data_t data1 = {"John", 25, 70.5, 180.0};
  List_Insert_First(&seznam1, data1);
  Data_t data2 = {"Alice", 30, 65.3, 165.0};
  List_Insert_First(&seznam1, data2);

  Data_t data3 = {"Bob", 40, 80.0, 175.0};
  List_Insert_First(&seznam2, data3);
  Data_t data4 = {"Eve", 35, 55.2, 160.0};
  List_Insert_First(&seznam2, data4);
  ...
  while (running) {
    ...
    switch (toupper(c)) {
      ...
      case 'm':
        ...
        // Pridany kod pro testovani funkce Merge
      case 'Z':
        printf("Merge - Merge the lists\n");
        Merge(&vysledek, &seznam1, &seznam2);
        printf("Merged list:\n");
        Vypis_Seznam(vysledek);
        break;
