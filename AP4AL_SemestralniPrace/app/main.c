...
int main(void)
{
  ...
  // Pridany kod na zaklade zadani BubbleSort - neserazeny seznam pro otestovani funkcnosti programu
  strcpy(data.name, "Jan");
  data.age = 25;
  data.weight = 70;
  data.height = 175;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "Alena");
  data.age = 30;
  data.weight = 60;
  data.height = 165;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "Bohumil");
  data.age = 20;
  data.weight = 80;
  data.height = 180;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "Eva");
  data.age = 22;
  data.weight = 68;
  data.height = 170;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "Karel");
  data.age = 28;
  data.weight = 72;
  data.height = 178;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "František");
  data.age = 24;
  data.weight = 75;
  data.height = 182;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "Anna");
  data.age = 26;
  data.weight = 65;
  data.height = 168;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "Helena");
  data.age = 23;
  data.weight = 62;
  data.height = 160;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "Ivan");
  data.age = 27;
  data.weight = 77;
  data.height = 185;
  List_Insert_First(&seznam, data);

  strcpy(data.name, "Jana");
  data.age = 29;
  data.weight = 63;
  data.height = 163;
  List_Insert_First(&seznam, data);
  ...
  while (running) {
    ...
    switch (toupper(c)) {
      ...
      case '9':
        ...
        // Pridany kod na zaklade zadani BubbleSort - moznost pro otestovani funkcnosti programu
      case 'B':
        printf("Bubble_Sort - Serad list pomoci BubbleSort metody\n");
        List_BubbleSort(&seznam);
        break;
